...
// Placeholder for the user's actual modules
const userModules = ref([
    { id: 1, name: 'Tesla (TSLA)' },
    { id: 2, name: 'NVIDIA (NVDA)' },
    { id: 3, name: 'Bitcoin (BTC)' },
]);
    ...
    <div class="space-y-8">
        ...
        <div class="p-6 bg-white/80 dark:bg-gray-900/80 backdrop-blur-md rounded-2xl shadow-lg border border-black/5 dark:border-white/10">
            <div class="flex justify-between items-center mb-4">
                <h2 class="text-xl font-bold text-gray-800 dark:text-gray-200">Your Modules ({{ userModules.length }} / {{ moduleLimit }})</h2>
                <Button v-if="canAddModule" label="Add Module" icon="pi pi-plus" severity="primary" />
                <Button v-else label="Upgrade to add more" icon="pi pi-arrow-up" severity="secondary" disabled />
            </div>
            <div v-if="userModules.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <div v-for="module in userModules" :key="module.id" class="p-4 bg-gray-100 dark:bg-gray-800 rounded-lg">
